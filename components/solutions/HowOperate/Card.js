...
const Card = ({ data }) => {
  return (
    <div className="bg-white p-[25px] drop-shadow-[1px_0px_18px_rgba(0,0,0,0.07)] rounded-[10px] relative">
      ...
      <div className="text-[19px] font-medium tracking-[0.03em] leading-[33px] mb-[20px] text-center">
        {data.title}
      </div>
      <ul className="text-[14px] text-[#7b86a2] tracking-[0.03em]">
        {data.list_rep.map((item) => (
          <li
            className="leading-[19px] mb-[15px] flex items-start"
            key={item.id}
          >
      ...
      <div
        className={twclsx(
          'ball w-[17px] h-[17px] rounded-full bg-secondary border-[2px] border-[#f1f2f4] absolute -bottom-[56px] left-0 right-0 mx-auto',
          {
